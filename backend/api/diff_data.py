...
def get_color_json(data: dict, cloc: str) -> dict:
    ...
    # Instead of computing diffs from raw meter data, read precomputed values from calc.json
    # Expected timestamp format in calc.json is YYYY-MM-DD HH:MM:SS
    from datetime import datetime

    dt = datetime.strptime(f"{day} {hour}", "%d.%m.%Y %H:%M:%S")
    t2 = dt.strftime("%Y-%m-%d %H:%M:%S")

    # Load calc.json which contains per-location Import/Export values at timestamps
    calc_path = os.path.join(DATA_DIR, "calc.json")
    calc_map = {}
    if os.path.exists(calc_path):
        try:
            with open(calc_path, "r", encoding="utf-8") as fh:
                calc_list = json.load(fh)
            # calc_list is a list of { location: { timestamp: {Import,Export}, ... }, ... }
            for entry in calc_list:
                if isinstance(entry, dict):
                    for loc, times in entry.items():
                        calc_map[loc] = times
        except Exception:
            calc_map = {}

    # Build consumption mapping by reading Import at requested timestamp for each location
    location_consumption = {}
    for location, meters in location_to_meters.items():
        # Default to 0 if missing
        import_val = 0
        loc_times = calc_map.get(location)
        if isinstance(loc_times, dict):
            # loc_times keys should be timestamps like '2025-06-08 12:00:00'
            entry = loc_times.get(t2)
            if isinstance(entry, dict):
                import_val = entry.get("Import", 0)
        location_consumption[location] = import_val
